    ...
    if(gp.buttons[5]?.pressed && !this.prevShot){
      for(let i=0;i<this.targets.length;i++){
        if(hitCheck.call(this,this.targets[i])){
          removeTarget.call(this,i); addTarget.call(this); drawLines.call(this); addScore(1); break;
        }
      }
    }
...
function addTarget(){
  ...
  hb.className='hitbox'; hb.style.width=hb.style.height=`${this.hitDiameter}px`;
...
function removeTarget(i){ const t=this.targets[i]; t.el.remove(); t.hb.remove(); this.targets.splice(i,1); }
function hitCheck(t){ const cx=this.cx+this.circleSize/2, cy=this.cy+this.circleSize/2;
  const tx=t.x+this.targetSize/2, ty=t.y+this.targetSize/2;
...
/* 補助線：ターゲット間 & 撃った瞬間のアンカー */
function drawLines(){
  this.ctx.clearRect(0,0,this.aux.width,this.aux.height);
  if(this.targets.length<2) return;
  /* アンカー: 今回は撃った瞬間のカーソル中心 → targets */
  ...
  this.targets.forEach(t=>{
    const tx=t.x+this.targetSize/2, ty=t.y+this.targetSize/2;
    this.ctx.moveTo(ax,ay); this.ctx.lineTo(tx,ty);
  });
  this.ctx.stroke();
}
